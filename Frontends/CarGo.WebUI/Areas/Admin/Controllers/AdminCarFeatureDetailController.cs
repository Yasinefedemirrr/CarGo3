    ...
    public class AdminCarFeatureDetailController : Controller
    {
        ...
        public async Task<IActionResult> Index(List<ResultCarFeatureByCarIdDto> resultCarFeatureByCarIdDto)
        {
            ...
            foreach (var item in resultCarFeatureByCarIdDto)
            {
                ...
                {
                    var client = _httpClientFactory.CreateClient();
                    await client.GetAsync("http://localhost:7266/api/CarFeatures/CarFeatureChangeAvailableToTrue?id=" + item.CarFeatureID);

                }
                else
                {
                    var client = _httpClientFactory.CreateClient();
                    await client.GetAsync("http://localhost:7266/api/CarFeatures/CarFeatureChangeAvailableToFalse?id=" + item.CarFeatureID);
                }
            }
